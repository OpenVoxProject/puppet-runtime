project 'agent-runtime-main' do |proj|
  ...
  proj.setting :host_ruby, case
    ...
    when platform.is_cross_compiled? && platform.is_macos? then "/usr/local/opt/ruby@#{ruby_version_y}/bin/ruby"
  ...
  proj.setting :host, case
    ...
    when platform.is_cross_compiled? && platform.is_macos? then '--host aarch64-apple-darwin --build x86_64-apple-darwin --target aarch64-apple-darwin'
  end
